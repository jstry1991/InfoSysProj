...
$query ="SELECT med.rxNumber, med.drugName as med_name, med.dateFilled, med.directions, med.form, med.numberOfRefills, med.quantity, med.strength, med.originalDate, med.pharmacist, p.name as p_name, p.address, em.name as em_name, di.cost, di.description, di.warning, ins.company, ins.policyType 
...
while ($row = mysqli_fetch_assoc($result)){ //Creates a loop through results
	if ($row)
		echo "<tr><td>".$row["p_name"]."</td><td>".$row["address"]."</td><td>".$row["em_name"]."</td><td>".$row["rxNumber"]."</td><td>".$row["med_name"]."</td><td>".$row["directions"]."</td><td>".$row["originalDate"]."</td><td>".$row["dateFilled"]."</td><td>".$row["numberOfRefills"]."</td><td>".$row["strength"]."</td><td>".$row["quantity"]."</td><td>".$row["pharmacist"]."</td><td>".$row["cost"]."</td><td>".$row["description"]."</td><td>".$row["warning"]."</td><td>".$row["company"]."</td><td>".$row["policyType"]."</td></tr>";
}
}
else {
